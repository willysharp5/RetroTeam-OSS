...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ...
    className={classNames(
      `flex h-10 w-full bg-white items-center justify-between
...
>(({ className, children, sideOffset = 4, ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={classNames(
        `mt-11 animate-in fade-in-250 relative z-50 w-auto min-w-[8rem] overflow-hidden border border-transparent
...
const SelectItemClassName = `
  relative flex select-none items-center rounded-md hover:bg-primary-50
    ...
    <span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
      <SelectPrimitive.ItemIndicator>
        <Image
          src={checkIcon}
          alt="checkIcon"
          className="h-5 text-primary-500"
        />
      </SelectPrimitive.ItemIndicator>
    </span>
