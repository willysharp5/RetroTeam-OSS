import Head from 'next/head';
import Script from 'next/script';
import configuration from '~/configuration';
import GoogleAnalyticsScripts from '~/core/ui/GoogleAnalyticsScripts';
import { useContext } from 'react';
import { ThemeContext } from '~/core/contexts/theme';

const DemoLayout: React.FCC = ({ children }) => {
  const siteUrl = configuration.site.siteUrl;

  if (!siteUrl) {
    throw new Error(`Please add the property siteUrl in the configuration`);
  }

  const structuredData = {
    name: configuration.site.demoName,
    url: siteUrl,
    logo: `${siteUrl}/assets/images/favicon/favicon-150x150.png`,
    '@context': 'https://schema.org',
    '@type': 'Organization',
  };

  return (
    <>
      <Head>
        <title key="title">{configuration.site.name}: Demo</title>

        <link rel="shortcut icon" href="/assets/images/favicon/favicon.ico" />
        <link
          rel="apple-touch-icon"
          sizes="144x144"
          href="/assets/images/favicon/apple-touch-icon.png"
        />
        <link
          rel="icon"
          type="image/png"
          sizes="16x16"
          href="/assets/images/favicon/favicon-16x16.png"
        />
        <link
          rel="icon"
          type="image/png"
          sizes="32x32"
          href="/assets/images/favicon/favicon-32x32.png"
        />
        <link rel="manifest" href="/assets/images/favicon/site.webmanifest" />
        <link
          rel="mask-icon"
          href="/assets/images/favicon/safari-pinned-tab.svg"
          color="#000000"
        />
        <link rel="dns-prefetch" href="//fonts.googleapis.com" />
        <link rel="preconnect" href="https://fonts.gstatic.com" />
        <link rel="canonical" href={siteUrl} />
        <meta name="msapplication-TileColor" content="#ffffff" />
        <meta
          name="msapplication-config"
          content="/favicon/browserconfig.xml"
        />
        <MetaColor />

        <meta
          name="description"
          content={configuration.site.demoDescription}
          key="meta:description"
        />
        <meta
          property="og:title"
          key="og:title"
          content={`${configuration.site.demoName}`}
        />
        <meta
          property="og:description"
          key="og:description"
          content={configuration.site.demoDescription}
        />
        <meta property="og:site_name" content={configuration.site.siteName} />
        <meta property="twitter:title" content={configuration.site.siteName} />
        <meta property="twitter:card" content="summary_large_image" />
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
        <meta
          property="twitter:creator"
          content={configuration.site.twitterHandle}
        />
        <meta property="og:image" content={`/assets/svg/opneGraph.svg`} />
        <meta name="robots" content="noindex"></meta>
      </Head>

      <GoogleAnalyticsScripts />

      {/* JSON-LD Structured Data */}
      <Script
        id="structured-data"
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData) }}
        strategy="afterInteractive"
      />

      <main>{children}</main>
    </>
  );
};

export default DemoLayout;

function MetaColor() {
  const { theme } = useContext(ThemeContext);

  const color =
    theme === 'dark'
      ? configuration.site.themeColorDark
      : configuration.site.themeColor;

  return <meta name="theme-color" content={color} />;
}
