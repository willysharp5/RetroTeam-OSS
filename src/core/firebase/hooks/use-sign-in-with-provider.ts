...
export function useSignInWithProvider() {
  ...
  return { signInWithProvider, state, resetState, isAnonymous };
}

async function getCredential(auth: Auth, provider: AuthProvider) {
  const user = auth.currentUser;

  const { signInWithPopup, browserPopupRedirectResolver, linkWithPopup } =
    await import('firebase/auth');
  if (user) {
    return linkWithPopup(user, provider, browserPopupRedirectResolver);
  }
