...
async function updateOrganizationOwnerHandler(
  ...
  if (currentUserMembership.role !== MembershipRole.Admin) {
    return throwForbiddenException(`Current member is not admin`);
  }
  ...
  const updateData = {
    ...
    [`members.${targetUserId}.role`]: MembershipRole.Admin,
