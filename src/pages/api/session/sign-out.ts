...
/** Client cookie set when admin is impersonating; must be cleared on sign-out */
const IMPERSONATING_CLIENT_COOKIE = 'impersonating';
...
function destroySessionCookies(res: NextApiResponse) {
  ...
  destroyCookie({ res }, ORGANIZATION_ID_COOKIE_NAME, options);
  destroyCookie({ res }, TEAM_ID_COOKIE_NAME, options);
  destroyCookie({ res }, IMPERSONATING_CLIENT_COOKIE, options);
}
