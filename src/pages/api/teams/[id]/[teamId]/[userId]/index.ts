...
import { deleteMemberTeam } from '~/lib/server/teams/delete-member';
...
async function onJoinHandler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const result = await deleteMemberTeam(params);

    if (result.success === true) {
      res.send({ success: true });
    } else {
      res.status(404).send({ success: false, error: 'Member not found' });
    }
  } catch (error) {
    console.error(error);
