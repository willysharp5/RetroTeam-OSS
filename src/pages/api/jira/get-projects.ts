...
async function onJoinHandler(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        const response = await fetch(`${body.domain}/rest/api/3/project/search?maxResults=5&startAt=0&query=${body.text}`, {
            method: 'GET',
            headers: {
        ...
        if (!response.ok) {
            return res.status(response.status).json({ error: 'Failed to fetch Jira projects' });
        }
