  ...
  useEffect(() => {
    if (invite) {
      ...
      if (daysSinceCreation >= 14) {
        // Checking if 14 days have passed
        setExpired(true);
      }
    }
  }, [invite]);
    ...
    const promise = addMemberToOrganization(body).then(() => {
      if (!currentSession) {
        ...
        addUserData(Userbody)
          ...
          .catch((e) => {
            console.error('ERROR on InviteAccepted', e);
          });
      } else {
        redirectToHomePage();
      }
            ...
            <Fragment>
              ...
              <If condition={!currentSession}>
                ...
                <div>
                  <p className={'text-center text-zinc-500 text-sm mb-10'}>
                    You have been invited by {invite.facilitator} to join{' '}
                    {invite.organization.name}. No need to create an account.
                  </p>
                </div>
              </If>
...
export async function getServerSideProps(ctx: GetServerSidePropsContext) {
  ...
  try {
    ...
    if (userId) {
      ...
      // if yes, we redirect the user to the members page
      if (isPartOfOrganization) {
        deleteInviteByCode(code);
        if (userRole === MembershipRole.Admin) {
          return redirectToAdminPage(organizationId);
        } else {
          return redirectToMembersPage(organizationId);
        }
      }
    }
...
function redirectToAdminPage(organizationId: string) {
  return {
    redirect: {
      permanent: false,
      destination: `/${organizationId}/settings/organization`,
    },
  };
}

function redirectToMembersPage(organizationId: string) {
  return {
    redirect: {
      permanent: false,
      destination: `/${organizationId}/settings/organization/members`,
    },
  };
}
