...
import { getBoardInviteByCode } from '~/lib/server/board/memberships';
...
const InvitePage = (
  ...
  const { trigger: addMemberToOrganization } = useAddMemberToBoard(
    organization.id,
  );
  ...
  const redirectToTeamBoardPage = useCallback(() => {
    const path = getTeamDeepLinkPath(
      invite.organization.id,
      `/board/${invite.retrospective.id}`,
    );
    return router.push(path);
  }, [router, invite]);

  function getTeamDeepLinkPath(organizationId: string, path: string) {
  ...
  const redirectToSignUpPage = useCallback(() => {
    ...
    const tokenBoard = btoa(invite.retrospective.name);
    const tokenBoardId = btoa(invite.retrospective.id);
    const tokenTeamBoardId = btoa(invite.teamId);
    ...
    Cookies.set('boardInvite', tokenBoard);
    Cookies.set('boardIdInvite', tokenBoardId);
    ...
    Cookies.set('boardTeam', tokenTeamBoardId);
  ...
  const onInviteAccepted = useCallback(async () => {
    const body = {
      code: invite.code,
      type: invite.retrospective.type,
      name,
      lastName,
      retrospectiveId: invite.retrospective.id,
  ...
  if (loading) {
    return (
      <PageLoadingIndicator>
        <Trans
          i18nKey={'auth:addingToBoard'}
          values={{ name: invite.retrospective.name }}
          components={{ b: <b /> }}
        />
      </PageLoadingIndicator>
    );
              ...
              <If condition={currentSession}>
                ...
                <div>
                  <p className={'text-center text-zinc-500 text-sm mb-10'}>
                    ...
                    {organization.name} - {invite.retrospective.name}.
                  </p>
                </div>
              </If>
              <If condition={!currentSession}>
...
export async function getServerSideProps(ctx: GetServerSidePropsContext) {
  ...
  try {
    const inviteRef = await getBoardInviteByCode(code);
    ...
    // We check if the user is already part of the board
    if (userId) {
      const board = await getBoardById(invite.retrospective.id);
      const isPartOfBoard = board.members[userId];
      if (isPartOfBoard) return redirectToBoardPage(invite);
    }
...
function redirectToBoardPage(invite: BoardMembershipInvite) {
  return {
    redirect: {
      permanent: false,
      destination: `/board/${invite.retrospective.id}`,
    },
  };
}
