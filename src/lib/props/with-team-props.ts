...
import { getTeamById, getUserData } from '~/lib/server/queries';
...
import { getUserRoleByTeam } from '../server/organizations/memberships';
 ...
 * @param options
 */
export async function withTeamProps(
  ...
  try {
    ...
    const teamId = ctx.params?.id as Maybe<string>;
    const currentOrganizationId = ctx.req.cookies[
      ORGANIZATION_ID_COOKIE_NAME
    ] as string;

    // if the team wasn't provided we cannot continue
    // so, we redirect to 404
    if (!teamId) {
      return notFound();
    }

    const teamRef = await getTeamById(currentOrganizationId, teamId);
    const teamData = teamRef?.data();

    // if the team wasn't found, it's 404
    if (!teamData) {
      logger.warn(
        {
          teamId,
        },
        `User navigated to team detail page, but it wasn't found. Redirecting to home page...`,
      );

      return notFound();
    }

    // We check if the user is already part of the organization - team
    if (userId) {
      ...
      // if no, we redirect the user to the error page
