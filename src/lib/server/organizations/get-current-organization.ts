...
async function getFirstOrganization(userId: string) {
  try {
    ...
    const organizations = await getOrganizationAdminByUserId(userId).limit(1).get();
    if (!organizations.empty) {
      const doc = organizations.docs[0];
      return serializeOrganizationData(doc.data(), doc.id);
    } else {
      const firstOrganizations = await getOrganizationsByUserId(userId).limit(1).get();
      const doc = firstOrganizations.docs[0];
      return serializeOrganizationData(doc.data(), doc.id);
    }
...
/**
 * @name getOrganizationAdminByUserId
 * @description Get all the organizations where the user {@link userId} is an admin
 * @param userId
 */
function getOrganizationAdminByUserId(userId: string) {
  const organizations = getOrganizationsCollection();
  const path = `members.${userId}.role`;

  return organizations.where(path, '==', 1);
}
