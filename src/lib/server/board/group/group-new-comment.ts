 ...
 */
export async function groupComment({
  id,
  organization,
  retrospectiveId,
  comment,
  ...
  if (!id || !organization || !retrospectiveId || !comment) {
  ...
  const commentPath = `/${ORGANIZATIONS_COLLECTION}/${organization}/board/${retrospectiveId}/comments/${comment}`;
