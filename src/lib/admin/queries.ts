...
async function getUserRefById(id: string) {
  const result = await getUsersCollection().doc(id).get();

  const userSnapshot = result.data();
...
export async function getOrganizationMembersByOrganizationId(
  ...
  const requests = userIds.map((id) => {
    return new Promise<Result>(async (resolve, reject) => {
      ...
      const userData = (await getUserRefById(id)) as any;
