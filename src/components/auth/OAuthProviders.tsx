...
const OAuthProviders: React.FCC<{
  onSignIn: (email: string, id: string) => unknown;
}> = ({ onSignIn }) => {
  const {
    signInWithProvider,
  ...
  const createSession = useCallback(
    async (user: User) => {
      ...
      onSignIn(user.email as string, user.uid);
    },
  ...
  const onSignInWithProvider = useCallback(
    async (signInRequest: () => Promise<UserCredential | undefined>) => {
      try {
        ...
        if (isAnonymous) {
          ...
          const email = credential?.user.email as string;
          const Userbody = { email, id };
    ...
    [setMultiFactorAuthError, createSession, isAnonymous, linkUser],
  );
