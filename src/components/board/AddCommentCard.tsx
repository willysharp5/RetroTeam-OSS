...
import AddComments from '~/lib/board/hooks/use-add-comments';
...
export default function AddCommentCard({
  ...
  const { trigger: addCommentsData } = AddComments();
  ...
  const currentUserData = teamMembers?.find(
    (user: any) => user.userId === currentUserId,
  );
  const userName = currentUserData?.fullName as string;
