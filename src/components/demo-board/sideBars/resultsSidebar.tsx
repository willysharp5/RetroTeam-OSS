...
import { ResultsSidebarProps } from '~/lib/board/types/types';
import Results from '~/components/retrospectives/Results';
...
export default function ResultsSidebar({
  ...
  const onRedirectToResultsPage = useCallback(async () => {
    try {
      await updateMemberStatus(
        ...
        userId,
        'href',
        `/results/${retrospective.id}`
      );
    } catch (e) {
      console.error(e);
    }
  }, [updateMemberStatus, organizationId, retrospective, teamId, userId]);
  ...
  return (
    <Sidebar>
      <div className="w-full md:w-[650px] print:w-full z-50 bg-white h-full shadow-md border-l fixed top-0 right-0 print:overflow-hidden overflow-y-auto">
        ...
        <div className="pt-0 p-6">
          <Results hasHeader={false} printRef={printRef} />
        </div>
      </div>
    </Sidebar>
  );
