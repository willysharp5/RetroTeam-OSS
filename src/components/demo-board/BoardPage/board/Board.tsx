...
function Board({
  ...
  useEffect(() => {
    ...
    const groupsWithType = groups.map((group) => ({ ...group, type: 'group' }));
  ...
  return (
    <div className="flex space-x-2 h-full w-full overflow-auto  bg-gray-100">
      <DndContext
        sensors={sensors}
      ...
      >
        <div className="w-full bg-gray-100">
          <div
            ...
            className={`bg-gray-100 w-full flex px-8 py-6 tv:px-36 tv:pb-[150px] justify-start  ${
              columns?.length <= 3 && 'xl:justify-center'
            } overflow-auto  items-start gap-2 lg:gap-20 py-3 px-4 `}
          >
            {columns?.map((col) => (
              <ColumnContainer
    ...
    if ((isActiveATask && isOverAGroup) || (isActiveAGroup && isOverATask)) {
      ...
      array.map((task: any, index: number) => {
        if (task.type === 'comment') {
          ...
          onUpdateGroup(task, index, task.status);
        }
      });
    }
