...
import useUpdateCurrentUser from '~/lib/server/user/update-current-user';
...
const UpdateEmailForm: React.FC<{ user: User }> = ({ user }) => {
  ...
  const { updateUserData } = useUpdateCurrentUser();

  const updateEmailWithCredential = useCallback(
    async (credential: UserCredential, email: string) => {
      try {
        // First, update the user's email address
        await updateEmail(credential.user, email);

        // Then, update user data with the new email
        await updateUserData({ email });

        // Show success toast notification
        toast.success(t('profile:updateEmailSuccess'));

        // Create a server-side session after email update
    ...
    [createServerSideSession, setErrorMessage, t, updateUserData],
  );
